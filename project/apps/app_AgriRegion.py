...
#wrap all your code in this method and you should be done
    def run(self):
        ...
        cursor=db.cursor()
        sql = "select region,years,percentage from agricultural_by_region"
        cursor.execute(sql)
        res = cursor.fetchall()
        ...
        subtitle = '<p style="font-family:sans-serif; font-size: 20px;color:#008080;">Agricultural Land(Region)</p>'
        st.markdown(subtitle, unsafe_allow_html=True)


        regions = st.multiselect("Choose regions to visualize agricultural land percentage",all_regions , all_regions[29:32])

        space(1)

        source = source[source.Region.isin(regions)]
        chart = chart_AgriRegion.get_chart(source)
